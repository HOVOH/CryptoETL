...
describe("moneyConvert", () =>{
    ...
    it("should trucates number of decimal", () => {
        expect(moneyToBase(1.0123456789012, MoneyUnits.DOLLARS)).to.eq(10123456789)
    })

    it("converts $ to base", () => {
        expect(moneyToBase(1.5, MoneyUnits.DOLLARS)).to.eq(15000000000)
    })

    it('convert cents to base', function () {
        expect(moneyToBase(1.5, MoneyUnits.CENTS)).to.eq(150000000)
    });

    it('should convert base to $', function () {
        expect(baseToMoney(15000000000, MoneyUnits.DOLLARS)).to.eq(1.5);
    });

    it('should convert base to cents', function () {
        expect(baseToMoney(150000000, MoneyUnits.CENTS)).to.eq(1.5);
    });
    ...
    it("Converts cents to base", () => {
        expect(moneyConvert(20.1, MoneyUnits.CENTS, MoneyUnits.BASE)).to.eq(2010000000);
    })
})
